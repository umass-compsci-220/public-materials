...
3. Write a function `statefulObserver` that takes an `Observable<number>` `o` as input returns a new `Observer<number>` which only updates if the current update value from `o` is divisible by the previous update value from `o`.
