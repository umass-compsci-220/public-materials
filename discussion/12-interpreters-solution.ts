...
export function interpExpressionAsync(s: State, e: AsyncExpression): Promise<boolean> {
  switch (e.kind) {
    ...
    case "variable":
      return e.name in s ? Promise.resolve(s[e.name]) : Promise.reject("undefined variable");
    case "operator":
      return interpExpressionAsync(s, e.left).then(b1 =>
        e.operator === "&&"
          ? b1
            ? interpExpressionAsync(s, e.right)
            : false
          : b1
          ? true
          : interpExpressionAsync(s, e.right)
